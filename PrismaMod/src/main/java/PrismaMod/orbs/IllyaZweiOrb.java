...
public class IllyaZweiOrb extends AbstractOrb {

    // Standard ID/Description
    public static final String ORB_ID = DefaultMod.makeID("IllyaZweiOrb");
    private static final OrbStrings orbString = CardCrawlGame.languagePack.getOrbString(ORB_ID);
    public static final String[] DESC = orbString.DESCRIPTION;

    private static final Texture IMG = TextureLoader.getTexture(makeOrbPath("Zwei_orb.png"));
    ...
    public void applyFocus() {
        ...
        if (p.hasPower(IllyaCasterPower.POWER_ID)){
            ...
            evokeAmount = (int) (baseEvokeAmount * 1.5);
        ...
        if (p.hasPower(UpgradeOrbsPower.POWER_ID)){
            ...
            evokeAmount = (int) (evokeAmount * 1.5);
        }
    ...
    public void onEvoke() {
        ...
        //Find the first boss in the room
        for (int i = 0; i < m.monsters.size(); i++) {
            AbstractMonster targ = m.monsters.get(i);
            if (targ.type == AbstractMonster.EnemyType.BOSS) {
                targetIndex = i;
                break;
            }
        }

        if (targetIndex > 0) { //If there is a boss in the room do double damage to him
            AbstractMonster target = m.monsters.get(targetIndex);
            DamageInfo info = new DamageInfo(target, evokeAmount);
            AbstractDungeon.actionManager.addToTop(new DamageAction(target, info));
        } else {    //If no boss then just do normal damage to a random enemy
    ...
    public void render(SpriteBatch sb) {
        sb.setColor(new Color(0.0f, 0.15f, 0.9f, c.a / 2.0f));
        sb.draw(img, cX - 48.0f, cY - 48.0f + bobEffect.y, 48.0f, 48.0f, 96.0f, 96.0f, scale + MathUtils.sin(angle / PI_4) * ORB_WAVY_DIST * Settings.scale, scale, angle, 0, 0, 96, 96, false, false);
        sb.setColor(new Color(1.0f, 0.3f, 1.0f, this.c.a / 2.0f));
