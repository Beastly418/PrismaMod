...
public class IllyaAttackOrb extends AbstractOrb {

    // Standard ID/Description
    public static final String ORB_ID = DefaultMod.makeID("IllyaAttackOrb");
    ...
    public void onEvoke() {
        ...
        DamageInfo info = new DamageInfo(m, evokeAmount);

        AbstractDungeon.actionManager.addToTop(new DamageAction(m, info));

        if (AbstractDungeon.player.hasPower(IllyaForeignerPower.POWER_ID)) {
            AbstractDungeon.actionManager.addToTop(new ApplyPowerAction(m, m, new PoisonPower(m, m, evokeAmount)));
        }
    ...
    public void render(SpriteBatch sb) {
        sb.setColor(new Color(0.35f, 0.0f, 0.0f, c.a / 2.0f));
        sb.draw(img, cX - 48.0f, cY - 48.0f + bobEffect.y, 48.0f, 48.0f, 96.0f, 96.0f, scale + MathUtils.sin(angle / PI_4) * ORB_WAVY_DIST * Settings.scale, scale, angle, 0, 0, 96, 96, false, false);
        sb.setColor(new Color(0.7f, 0.0f, 0.0f, this.c.a / 2.0f));
