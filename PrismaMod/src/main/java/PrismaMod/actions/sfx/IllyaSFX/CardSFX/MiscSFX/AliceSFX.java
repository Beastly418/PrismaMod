...
public class AliceSFX extends AbstractSFX {
    ...
    public AliceSFX(AbstractCreature target){
        this.images = new Texture[4];
        this.img = this.images[0] = TextureLoader.getTexture(makeSFXPath("All_Suits.png"));
        this.color = new Color(1.0F, 1.0F, 1.0F, 0.0F);// 24
        this.scale = .3F * Settings.scale;// 29
        this.target = target;
        ...
        for (int i = -1; i < pictures.length-1; i++){
            pictures[i+1] = new picture(img, AbstractDungeon.player.hb_w + i*target.hb_w, AbstractDungeon.floorY*1.5F + 150F, this.color);
        }
    ...
    public void render(SpriteBatch sb) {
        for(picture g: pictures){
            sb.setColor(g.color);
            sb.draw(g.image, g.x, g.y, 0, 0, g.image.getWidth(), g.image.getHeight(), scale, scale, g.rotation, 0, 0, g.image.getWidth(), g.image.getHeight(), false, false);
        }
    }
