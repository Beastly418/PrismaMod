...
import PrismaMod.actions.MonsterActions.Gilgamesh.sfx.EnumaElisSFX;
...
public class EnumaElis extends AbstractMonsterAction {

    public DamageInfo attack;
    public int numAttack;
    public EnumaElis(Gilgamesh m, AbstractCreature target, DamageInfo attack, int numAttack){
        this.m = m;
        this.handler = getHandler(m);
        this.target = target;
        this.sfx = new EnumaElisSFX(m, (AbstractPlayer)target);
    ...
    public void update() {
        ...
        if(this.delay <= 0 && !hit){
            for(int i = 0; i < this.numAttack; i++)  //Hit 5 times
